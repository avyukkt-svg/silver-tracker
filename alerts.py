...
def check(score_data, market):
    ...
    if signal and cfg["enabled"] and (signal != last_signal or cooled):
        ...
        body = (
            f"Silver Buy Score: {score}/100  ({score_data['label']})\n"
            f"Signal: {verb}\n"
            f"Price:  {price:,} {cur}\n\n"
            "Why:\n - " + "\n - ".join(score_data["reasons"]) + "\n\n"
            f"News bias: {score_data['components']['news']:+} | "
            f"Levels: {score_data['components']['levels']:+} | "
            f"Momentum: {score_data['components']['momentum']:+}\n\n"
            "— Silver Tracker (educational, not financial advice)"
        )
        if _smtp_ready(cfg):
            try:
            ...
            detail = f"{verb} signal ({score}) — SMTP not configured, logged only"
        _log(detail)
